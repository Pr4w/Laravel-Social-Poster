...
Connection-level failures (cURL timeouts, DNS errors, refused connections, TLS) never produce a response, so a driver's `classifyError()` never sees them. These are caught in the publish job and `postNow()` for every platform and turned into a `TemporaryException` with reason `Timeout`, with the original exception chained as `previous`, so a network blip on any driver is retried with backoff rather than surfacing as a raw stack trace. Note the interaction with idempotency: a timeout on a create or publish call may mean the platform actually processed the request, so the retry could double-post unless the database idempotency store is enabled, which is what makes retrying these safe.
