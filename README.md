...
On success a `PostResult` carries `platformPostId` (the platform's own id, for analytics and later lookups) and `url`, a clickable public link to the post. Instagram and Threads fetch their permalink right after publishing; X, LinkedIn, and YouTube derive it from the id. The permalink fetch is best-effort, so if it ever fails the result still has the id, just with a null `url`.

## Credentials

Pass credentials per call with `->using()`, or set them in `config/social.php` under `platforms`. Token acquisition and refresh are out of scope: the package is a pure publishing engine and expects a current token.# Social Poster
